...
export class SupabaseService {
   ...
   * of reading `ready`, so a signed-in user never sees the sign-in screen flash past on
  ...
  /**
   * Sends the recovery link. Resolves the same way whether or not the address is
   * registered — GoTrue is deliberately quiet here and the UI must stay quiet too,
   * or the screen becomes an account-enumeration oracle.
   *
   * `redirectTo` must be on the project's redirect allow-list (Supabase dashboard →
   * Authentication → URL Configuration), otherwise the link lands on `site_url`.
   */
   ...
   * masthead and any future signup read) and the `profiles` row (which co-members read).
   * The tenant name is left alone on purpose — it was seeded from this name at signup
   * but is a separate thing the moment a second person joins.
   */
  async updateDisplayName(displayName: string): Promise<void> {
    ...
    await this.run(
      this.client.from('profiles').update({ display_name: displayName }).eq('id', user.id),
    );
  }
