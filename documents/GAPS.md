...
| G-37 | **Custom pipeline stages** | **BUILT 2026-08-04** on `feat/stages-and-automations`, all nine migrations applied to dev. `public.lead_status` no longer exists; stages are per-tenant `pipeline_stages` rows with a reserved `kind` (open/won/lost) so a rename changes no arithmetic. Exactly-one-won/lost and at-least-one-open are enforced by a deferrable constraint trigger, not by convention. Reorder and archive are atomic RPCs. Attention rules generalised: `expects_reply` replaces the hardcoded proposal stage, first-open-by-position replaces `'new'`, per-stage `drift_days` replaces the `DRIFT_DAYS` map. Not visually verified — see G-40. | backend + screen | **closed** |
| G-38 | **Per-stage automations, tiers A and B** | **BUILT 2026-08-04**, applied to dev. Five actions: `set_reminder`, `add_note`, `assign_member`, `suggest_advance` (offers a move, never performs one — user decision, 2026-08-03) and `webhook`. Tier A runs inline in the stage-change transaction, so a rolled-back move sends nothing. Tier B sends via `pg_net` + `pg_cron` — **no Edge Function, nothing deployed**, so ARCHITECTURE §6's "no server logic exists" still holds. Idempotency keyed on the stage-move activity id; loop guard; demo leads never fire; idle runs re-check their precondition and skip with a Hebrew reason. Not visually verified — see G-40. | backend + screen | **closed** |
| G-40 | **Neither new surface has been opened by a human** | Same standing lesson as G-17 and G-33, now covering the four screens 9.1–9.4 plus every screen the stage refactor touched (day sheet, register, board, lead sheet, insights). 85 unit tests green, `ng build` green, detector clean, all RPCs verified present on the live schema by regenerated types — and not one pixel seen. Scenarios: `MANUAL-TESTS.md` §7. | decision | open |
| G-41 | **`pg_net` DNS-rebinding gap — a release condition, not a someday** | The webhook URL guard is a *string* check because `pg_net` cannot resolve a hostname before sending, so a webhook aimed at a domain whose A record answers `169.254.169.254` would still be sent. Currently near-theoretical: only a tenant owner can create an automation, pointing at their own endpoint. **Before the first external tenant**, either move the sender into an Edge Function that resolves and validates first, or restrict webhook hosts to an owner-confirmed allowlist. Recorded in the migration header too. | backend | **blocks first external tenant** |
| G-42 | **`pg_net` redirect behaviour unverified** | If `pg_net` follows redirects, a `302` to `127.0.0.1` defeats the URL guard entirely. Not verified against the live extension — no query tool in the session that built this. Confirm, and refuse redirects explicitly if it does. | backend | open |
| G-39 | **Inbound HTTP endpoint — the one thing that will force an Edge Function** | Not needed by either planned phase, and worth recording so the question is not re-argued each time. Postgres can *send* (`pg_net`); it cannot *receive*. Three future things need a public endpoint: email bounce and complaint handling (G-35), WhatsApp delivery receipts and inbound messages (G-36 — the 24-hour window opens on the recipient's message, so we must be told it arrived), and the public capture form (SCREENS 8.2, already named `capture-form` in ARCHITECTURE §6). The first one built pays the deploy cost that G-14 has been deferring. | backend | **closed 2026-09-18** — built as `capture-lead` for the Netlush WhatsApp bot (documents/PLAN-capture-lead.md); G-35/G-36 still need their own endpoints but the deploy path now exists. |
