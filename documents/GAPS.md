...
| G-30 | **Stale statements in the docs themselves** | **Partly closed 2026-08-03.** SCREENS.md is now reconciled against the code: 1.3, 1.6, 2.1, 2.2, 2.3, 2.5, 2.6, 3.1, 5.1, 5.3, 6.1, 6.6, 7.3, the §6 footer, the count table (32 → 35 built, 7 → 4 unbuilt) and the closing "nearest next screen" paragraph, which named a screen that had already been built. **Still open:** `PRODUCT.md:73` claims the app runs "against authored demo data in `client/src/app/core/demo-leads.ts` — no Supabase connection yet"; that file does not exist and the store reads the cloud dev project. | wiring | open |
